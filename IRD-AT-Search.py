    ...
    for proportion in [0.05]:
        for task_weight in [0.05]:
            for sequence in sequence_list:
                    ...
                    save_path = 'IRD-AT-' + str(weight) + '-' + str(proportion) + '-' + str(task_weight) + '-' + item1[0] + item2[0] + item3[0] + item4[0]
                    os.system(
                        'python3 IRD-AT.py --attribute1 %s --attribute2 %s --attribute3 %s --attribute4 %s --save_path %s --weight %f --proportion %f --task_weight %f' % (item1, item2, item3, item4, save_path, weight, proportion, task_weight)
                    )