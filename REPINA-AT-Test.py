...
def train_function(model, train_dl, test_iter, attribute, pre_model):
    if torch.cuda.is_available():
        model.to(DEVICE)
    ...
    for epoch in range(1, GCONF.epochs + 1):
        for batch in train_dl:
            ...
            pre_model.eval()
            pre_logits, pre_all_sentence_v, gender_logits, age_logits, country_logits, ethnicity_logits = pre_model(input_ids, attn_mask, token_type_ids)
            sp_loss = torch.nn.functional.mse_loss(all_sentence_v, pre_all_sentence_v)
            loss = loss - GCONF.weight * loss_at + GCONF.task_weight * sp_loss
            loss.backward()
...
for filename in glob.glob('/date3/lnk/DynamicFairness-Model/Sequence-AT-Replay-REPINA-0.1-0.1-0.1-*'):
