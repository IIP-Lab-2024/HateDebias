...
Options:
    ...
    --attribute3=<str>              attribute3
    --attribute4=<str>              attribute4
...
def train_function(model, train_dl, test_iter, attribute):
    if torch.cuda.is_available():
        model.to(DEVICE)
...
for attribute in [GCONF.attribute1,GCONF.attribute2,GCONF.attribute3,GCONF.attribute4]:
    if attribute == 'age':
        train_function(model, age_train_dl, val_dl, attribute)
    if attribute == 'country':
        train_function(model, country_train_dl, val_dl, attribute)
    if attribute == 'gender':
        train_function(model, gender_train_dl, val_dl, attribute)
    if attribute == 'ethnicity':
        train_function(model, ethnicity_train_dl, val_dl, attribute)
