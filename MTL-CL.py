...
def train_function(model, age_train_dl, country_train_dl, gender_train_dl, ethnicity_train_dl, test_iter):
    ...
    for epoch in range(1, GCONF.epochs + 1):
        for batch in age_train_dl:
            ...
            loss_cl = loss_cl(hidden, batch['age'].to(DEVICE).view(-1))
        ...
        for batch in country_train_dl:
            input_ids, attn_mask, token_type_ids, label, gender, age, country, ethnicity = batch['input_ids'], batch[
                'attn_mask'], batch['token_type_ids'], batch['label'], batch['gender'], batch['age'], batch['country'], \
                                                                                           batch['ethnicity']
            ...
            hidden = normalize(all_sentence_v.unsqueeze(1), dim=-1)
            loss_cl = SupConLoss(contrast_mode='all').to(DEVICE)
            loss_cl = loss_cl(hidden, batch['country'].to(DEVICE).view(-1))
            loss_cl = loss_cl / len(all_sentence_v)
        ...
        for batch in gender_train_dl:
            ...
            hidden = normalize(all_sentence_v.unsqueeze(1), dim=-1)
            loss_cl = SupConLoss(contrast_mode='all').to(DEVICE)
            loss_cl = loss_cl(hidden, batch['gender'].to(DEVICE).view(-1))
            loss_cl = loss_cl / len(all_sentence_v)
        ...
        for batch in ethnicity_train_dl:
            ...
            hidden = normalize(all_sentence_v.unsqueeze(1), dim=-1)
            loss_cl = SupConLoss(contrast_mode='all').to(DEVICE)
            loss_cl = loss_cl(hidden, batch['ethnicity'].to(DEVICE).view(-1))
            loss_cl = loss_cl / len(all_sentence_v)
...
train_function(model, age_train_dl, country_train_dl, gender_train_dl, ethnicity_train_dl, val_dl)
