...
for weight in [0.05, 0.15, 0.3]:
    for sequence in sequence_list:
        item1, item2, item3, item4 = sequence
        save_path = 'Fine-Tune-AT-'+ str(weight) + item1[0] + item2[0] + item3[0] + item4[0]
        os.system(
            'python3 Fine-Tune-AT.py --attribute1 %s --attribute2 %s --attribute3 %s --attribute4 %s --save_path %s --weight %f' % (item1, item2, item3, item4, save_path, weight)
        )