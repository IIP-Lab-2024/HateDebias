...
def train_function(model, train_dl, test_iter, attribute, pre_model):
    ...
    for epoch in range(1, GCONF.epochs + 1):
        for batch in train_dl:
            input_ids, attn_mask, token_type_ids, label, gender, age, country, ethnicity, task_id = batch['input_ids'],  batch[ 'attn_mask'], \
                                                                                                    batch[ 'token_type_ids'],  batch['label'], \
                                                                                                    batch['gender'],  batch['age'], batch['country'], \
                                                                                                    batch['ethnicity'], batch['task_id']

            if torch.cuda.is_available():
                input_ids, attn_mask, token_type_ids, label, gender, age, country, ethnicity, task_id = input_ids.to(
                    DEVICE), attn_mask.to(DEVICE), token_type_ids.to(DEVICE), label.to(DEVICE), gender.to(
                    DEVICE), age.to(DEVICE), country.to(DEVICE), ethnicity.to(DEVICE), task_id.to(DEVICE)
            ...
            loss = loss - GCONF.weight * loss_cl + GCONF.task_weight * sp_loss
            loss.backward()
...
model.to(DEVICE)
original_model = SentimentBertModel(GCONF.bert_path)
...
for attribute_num, attribute in enumerate(attribute_list):
    pre_model = train_function(model, dataset['dl'][attribute], val_dl, attribute, original_model)
