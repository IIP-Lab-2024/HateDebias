...
Options:
    ...
    --save_path=<str>              save_path [default: Sequence-CL-Replay-0.05-0.05ecag]
...
def train_function(model, train_dl, pre_task_train_iter, test_iter, attribute):
    if torch.cuda.is_available():
        model.to(DEVICE)
        ...
        if pre_task_train_iter != None:
            for pre_task in pre_task_train_iter:
                ...
                for batch in pre_task_train_dl:
                    input_ids, attn_mask, token_type_ids, label, gender, age, country, ethnicity = batch['input_ids'], batch['attn_mask'], batch['token_type_ids'], batch['label'], batch['gender'], batch['age'], batch['country'], batch['ethnicity']
...
for attribute_num, attribute in enumerate(attribute_list):
    if attribute_num == 0:
        train_function(model, dataset['dl'][attribute], None, val_dl, attribute)
    else:
        sample_dataset_dict = {}
        ...
        train_function(model, dataset['dl'][attribute], sample_dataset_dict, val_dl, attribute)
