...
def train_function(model, train_dl, pre_task_train_iter, test_iter, attribute, pre_model):
    ...
    for epoch in range(1, GCONF.epochs + 1):
        for batch in train_dl:
            ...
            loss_cl, exp_logits = loss_cl(hidden, batch[attribute].to(DEVICE).view(-1))
            ...
            if pre_model != None:
                ...
                pred_hidden = normalize(pre_all_sentence_v.unsqueeze(1), dim=-1)
                ird_mask = torch.ones((input_ids.shape[0],input_ids.shape[0]))
                loss_ird = SupConLoss(contrast_mode='all').to(DEVICE)
                _, exp_logits = loss_ird(hidden, mask=ird_mask)
                _, pre_exp_logits = loss_ird(pred_hidden, mask=ird_mask)

                loss_ = nn.CrossEntropyLoss()
                loss_ = loss_(exp_logits, pre_exp_logits) / ird_mask.shape[0]

                loss = loss + GCONF.task_weight * (loss_)
        ...
        if pre_task_train_iter != None:
            for pre_task in pre_task_train_iter:
                ...
                for batch in pre_task_train_dl:
                    ...
                    hidden = normalize(all_sentence_v.unsqueeze(1), dim=-1)
                    loss_cl = SupConLoss(contrast_mode='all').to(DEVICE)
                    loss_cl, exp_logits = loss_cl(hidden, batch[pre_task].to(DEVICE).view(-1))
                    loss_cl = loss_cl / len(all_sentence_v)
...
# for filename in glob.glob('Sequence-CL-SensitiveReplay-0.1-0.05-0.05-*'):
...
abr_map = {'c': 'country', 'a': 'age', 'e': 'ethnicity', 'g': 'gender'}
for filename in glob.glob('/date3/lnk/DynamicFairness-Model/Sequence-CL-Replay-IRD-0.1-0.1-0.1-*'):
    sequence_abr = filename.split('-')[-1]
    first_attribute = abr_map[sequence_abr[0]]
    second_attribute = abr_map[sequence_abr[1]]
    third_attribute = abr_map[sequence_abr[2]]
    last_attribute = abr_map[sequence_abr[-1]]
    GCONF.saved_model_path = filename
    DEVICE = torch.device('cuda:3' if torch.cuda.is_available() else 'cpu')

    model = torch.load(GCONF.saved_model_path + '/model_' + first_attribute + '.pth')
    model.to(DEVICE)
    print(filename, end='\t')
    test_eval(val_dl, model)

    model = torch.load(GCONF.saved_model_path + '/model_' + second_attribute + '.pth')
    model.to(DEVICE)
    print(filename, end='\t')
    test_eval(val_dl, model)

    model = torch.load(GCONF.saved_model_path + '/model_' + third_attribute + '.pth')
    model.to(DEVICE)
    print(filename, end='\t')
    test_eval(val_dl, model)

    model = torch.load(GCONF.saved_model_path + '/model_' + last_attribute + '.pth')
    model.to(DEVICE)
    print(filename, end='\t')
    test_eval(val_dl, model)
