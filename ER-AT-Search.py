...
for weight in [0.05, 0.2, 0.15]:
    for proportion in [0.05, 0.1, 0.15, 0.2]:
        for sequence in sequence_list:
            item1, item2, item3, item4 = sequence
            save_path = 'ER-AT-' + str(weight) + '-' + str(proportion) + item1[0] + item2[0] + item3[0] + item4[0]
            os.system(
                'python3 ER-AT.py --attribute1 %s --attribute2 %s --attribute3 %s --attribute4 %s --save_path %s --weight %f --proportion %f' % (item1, item2, item3, item4, save_path, weight, proportion)
            )
